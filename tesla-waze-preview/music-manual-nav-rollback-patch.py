...
new=r'''function resetManualMusicTransition(){
  try{clearTimeout(music.navWatchdog)}catch{}
  music.navWatchdog=null;
  music.navPending=0;
...
function directYoutubeTrack(track,reason='next'){
  ...
  music.current=track;music.userPaused=false;music.wantsPlayback=true;music.started=Date.now();
  if(prev&&mt(prev).id!==mt(track).id)mev(reason==='prev'?'skip':(reason==='auto'?'complete':'skip'),prev);
  mev('play',track);
  refreshCurrentMusicUi();setMusicPlaying(true);syncMediaSession();
  try{music.ytPlayer.loadVideoById(id,0,'default');music.ytPlayer.playVideo?.();return true}catch{return false}
}
