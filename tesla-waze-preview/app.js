...
function applyHeadingUp(center,zoom){
  ...
  const now=Date.now();
  if(state.heading!=null){const delta=state.lastAppliedHeading==null?180:Math.abs(((state.heading-state.lastAppliedHeading+540)%360)-180);if(delta>=3.5&&now-state.lastBearingAt>=650){if(typeof state.map.setHeading==='function')state.map.setHeading(state.heading,{ease:0,deadzone:3.5});else if(typeof state.map.setBearing==='function')state.map.setBearing(state.heading);state.lastAppliedHeading=state.heading;state.lastBearingAt=now}}
  if(!center)return;
...
function browserSpeak(text){if(!('speechSynthesis'in window)||!('SpeechSynthesisUtterance'in window))return;const synth=window.speechSynthesis;let done=false;const say=()=>{if(done)return;done=true;const v=synth.getVoices()||[],female=['laura','zuzana','vlasta','tereza','lucia','lucie','viktoria','victoria','alena','iveta','jana','katka','monika','zdenka','veronika','maria','marie','eva','hana','female','woman','žena','female voice'],male=['filip','martin','jakub','petr','peter','michal','tomas','tomáš','ondrej','matej','jiri','jan ','adam','daniel','david','male','man'];const name=x=>(x.name||'').toLowerCase(),isFemale=x=>female.some(n=>name(x).includes(n))&&!male.some(n=>name(x).includes(n)),isSlovak=x=>/^sk(?:-|_)/i.test(x.lang||''),isCzech=x=>/^(?:cs|cz)(?:-|_)/i.test(x.lang||'');const voice=v.find(x=>isSlovak(x)&&isFemale(x))||v.find(x=>isCzech(x)&&isFemale(x))||v.find(isSlovak)||v.find(isCzech)||v.find(isFemale)||v[0]||null,u=new SpeechSynthesisUtterance(text);if(voice){u.voice=voice;u.lang=voice.lang||'sk-SK'}else u.lang='sk-SK';u.rate=state.voiceMode==='soft'?.9:.96;u.pitch=state.voiceMode==='soft'?1.08:1;u.volume=1;try{synth.cancel();synth.resume()}catch{}synth.speak(u)};if((synth.getVoices()||[]).length)say();else{try{synth.addEventListener('voiceschanged',say,{once:true})}catch{}setTimeout(say,350)}}
async function speak(text,force=false){if(!state.voice&&!force)return;const normalized=normalizeSpeechText(text),generation=++voiceGeneration;unlockVoiceAudio();if(!cloudVoiceUnavailable){try{const key=`${state.voiceMode}:${normalized}`;let encoded=voiceCache.get(key);if(!encoded){const response=await fetch(PROD_ORIGIN+'/api/tts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:normalized,profile:state.voiceMode})});if(response.status===503)cloudVoiceUnavailable=true;if(!response.ok)throw Error('tts '+response.status);const data=await response.json();encoded=data.audioContent;if(typeof encoded!=='string')throw Error('tts audio');if(voiceCache.size>=30)voiceCache.clear();voiceCache.set(key,encoded)}if(generation!==voiceGeneration)return;const C=window.AudioContext||window.webkitAudioContext;if(C){voiceContext=voiceContext||new C();await voiceContext.resume();const bytes=Uint8Array.from(atob(encoded),c=>c.charCodeAt(0)),buffer=await voiceContext.decodeAudioData(bytes.buffer);if(generation!==voiceGeneration)return;try{activeVoiceSource?.stop()}catch{}const source=voiceContext.createBufferSource(),gain=voiceContext.createGain();gain.gain.value=4;source.buffer=buffer;source.connect(gain).connect(voiceContext.destination);activeVoiceSource=source;source.start();return}}catch(e){console.warn('Cloud navigation voice failed:',e?.message||e)}}if(generation===voiceGeneration)browserSpeak(normalized)}
...
function syncMediaSession(){updateMiniSeek();if(!('mediaSession' in navigator)||!music.current)return;try{navigator.mediaSession.metadata=new MediaMetadata({title:music.current.title||'Bez názvu',artist:music.current.artist||'',album:'Tesla Maps Smart Music',artwork:music.current.artwork?[{src:music.current.artwork}]:[]})}catch{}const actions={play:playMusic,pause:pauseMusic,previoustrack:mprev,nexttrack:mnext,seekbackward:()=>seekMusic(-15),seekforward:()=>seekMusic(15)};for(const [name,handler] of Object.entries(actions))try{navigator.mediaSession.setActionHandler(name,handler)}catch{}}
function wireAudio(){if(!music.audio)return;music.audio.onended=()=>{setMusicPlaying(false);if(music.current)mev('complete',music.current);if(music.autoNext)mnext()};music.audio.onplay=()=>setMusicPlaying(true);music.audio.onpause=()=>setMusicPlaying(false);music.audio.ontimeupdate=()=>{updateMiniSeek();if(!('mediaSession' in navigator)||!music.audio?.duration||!Number.isFinite(music.audio.duration))return;try{navigator.mediaSession.setPositionState({duration:music.audio.duration,playbackRate:music.audio.playbackRate||1,position:Math.min(music.audio.currentTime,music.audio.duration)})}catch{}}}
