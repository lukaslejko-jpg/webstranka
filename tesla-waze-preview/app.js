...
function renderTeslaNavigation(){
  ...
  const step=r.steps?.[p?.stepIdx||0],rawNext=r.steps?.[(p?.stepIdx||0)+1],arrived=!!p?.arrived,next=arrived?null:(/DESTINATION/i.test(String(rawNext?.opcode||''))&&((p?.remainingDistance??Infinity)>350)?null:rawNext);
...
function ensureMusicWindowControls(){
  ...
  const hs=document.createElement('div');hs.id='musicHeaderSearch';hs.className='music-header-search';hs.innerHTML='<input id="musicHeaderSearchInput" type="search" autocomplete="off" placeholder="Hľadať hudbu…" aria-label="Hľadať hudbu"><button id="musicHeaderSearchBtn" type="button" class="music-header-search-btn" aria-label="Hľadať">Hľadať</button><button id="musicHeaderSearchClose" type="button" class="music-header-search-close">Späť</button>';head.insertBefore(hs,head.querySelector('.spacer'));
  const runHeaderSearch=()=>{const shell=document.querySelector('.music-shell'),input=$('musicHeaderSearchInput'),full=$('musicSearch');if(!shell?.classList.contains('music-maximized')||!input||!full)return;full.value=input.value.trim();musicSearch()};
...
function setMusicSearchMode(active){document.querySelector('.music-tabs')?.classList.toggle('hidden',active);$('musicList')?.classList.toggle('hidden',active);const shell=document.querySelector('.music-shell'),results=$('musicSearchResults'),card=results?.closest('.music-card');if(card&&!card.id)card.id='musicSearchCard';shell?.classList.toggle('music-searching',!!active);if(!active&&results)results.innerHTML='';if(shell?.classList.contains('music-maximized')){const home=$('musicMaxHome');if(home)home.classList.toggle('hidden',!!active)}}
