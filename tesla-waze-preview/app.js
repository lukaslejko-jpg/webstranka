...
function normalizedOpcode(step){return String(step?.opcode||'').replace(/-/g,'_').toUpperCase()}
...
function nearestNavigation(p,coords,start=0){
  if(!coords?.length)return null;
  let best=null;const cos=111320*Math.cos(p.lat*Math.PI/180),kmh=(Number(state.speed)||0)*3.6;
  for(let i=Math.max(0,start);i<Math.min(coords.length-1,start+350);i++){
...
function confirmedOffRoute(distanceMeters,accuracy){return distanceMeters>Math.max(35,(Number(accuracy)||0)*1.5)}
...
function routeDestinationProjection(r){
...
function renderTeslaNavigation(){
  ...
  const step=r.steps?.[p?.stepIdx||0],nextIdx=nextActionableStepAfter(r,p?.stepIdx||0),rawNext=nextIdx>=0?r.steps?.[nextIdx]:null,arrived=!!p?.arrived,next=arrived?null:(/DESTINATION/i.test(String(rawNext?.opcode||''))&&((p?.remainingDistance??Infinity)>350)?null:rawNext);
  ...
  trip.innerHTML=`<div class="tesla-trip-stats"><b>${arr}</b><span>${fmtT(time)}</span><span>${fmtD(remaining)}</span></div><div class="tesla-trip-route">${esc(state.tripStartLabel||'Štart')} → ${esc(state.dest?.name||state.dest?.address||'Cieľ')}</div><div class="tesla-progress"><i style="width:${Math.round(ratio*100)}%"></i></div><div class="tesla-trip-actions"><button class="tesla-end" data-tesla-stop>Ukončiť trasu</button><button class="tesla-dots" data-tesla-settings aria-label="Nastavenia">•••</button></div>`;
...
function beginNavigationOverview(){
  ...
  const startAnchor=savedAnchorNear(state.pos,180)?.place;state.tripStartLabel=startAnchor?.name||startAnchor?.address||'Štart';state.tripKey=routeLearningKey(state.pos,state.dest);state.tripTrail=state.pos?[{...state.pos}]:[];state.tripOriginalRoute=(r.coords||[]).map(p=>({...p}));state.lastTrailAt=state.pos?{...state.pos}:null;/* NAV_TRIP_START_V96 */
...
function routeGeometryForward(r,start,end){
...
function routeDestinationAtEnd(r){return true}
...
function ensureMusicCompactHeader(){
  ...
  if(!head){
    ...
    $('musicCompactMin').onclick=()=>{const cfg=musicWindowState();const h=Math.max(360,Math.min(window.innerHeight-20,cfg.miniHeight||520));saveMusicWindow({minimized:false,maximized:false,height:h});$('musicMaxHome')?.remove();updateMiniSeek()};
...
function ensureMusicWindowControls(){
  ...
  min.onclick=()=>{const cfg=musicWindowState();const h=Math.max(360,Math.min(window.innerHeight-20,cfg.miniHeight||520));saveMusicWindow({minimized:false,maximized:false,height:h});$('musicMaxHome')?.remove();updateMiniSeek()};/* MUSIC_LAYOUT_NO_RESTART_V5 */
/* MUSIC_DIRECT_MINIMIZE_V78 */
  size.onclick=()=>{const cfg=musicWindowState();if(!cfg.maximized){saveMusicWindow({maximized:true,minimized:false});return}const sh=document.querySelector('.music-shell');if(!sh||!currentYoutubeId())return;const open=sh.classList.toggle('music-video-open');size.textContent=open?'Skryť video':'Video';syncTwMusicEngineVisibility();ensureMaxMusicHeaderSearch()};ads.onclick=()=>{const cfg=musicWindowState(),yt=currentYoutubeId();if(cfg.maximized&&yt)switchYoutubeToFree(yt,true)};/* MUSIC_V80_RESTORE_MINIMIZED_MAX_VIDEO */
/* MUSIC_V83_RESIZE_AND_ADS */
  syncMusicMinimizedHeader();
  const bindResize=(node,mode)=>node.addEventListener('pointerdown',e=>{e.preventDefault();node.setPointerCapture?.(e.pointerId);const sx=e.clientX,sy=e.clientY,sw=shell.getBoundingClientRect().width,sh=shell.getBoundingClientRect().height;const move=ev=>{if(mode==='w'||mode==='both'){const w=Math.max(340,Math.min(Math.min(window.innerWidth-20,760),sw+(sx-ev.clientX)));shell.style.width=w+'px'}if(mode==='h'||mode==='both'){const h=Math.max(360,Math.min(window.innerHeight-20,sh+(sy-ev.clientY)));shell.style.height=h+'px'}};const up=ev=>{node.releasePointerCapture?.(ev.pointerId);node.removeEventListener('pointermove',move);node.removeEventListener('pointerup',up);node.removeEventListener('pointercancel',up);(()=>{const rect=shell.getBoundingClientRect(),small=!musicWindowState().maximized;saveMusicWindow(small?{width:Math.round(rect.width),miniHeight:Math.round(rect.height),minimized:false,maximized:false}:{width:Math.round(rect.width),height:Math.round(rect.height),minimized:false,maximized:true})})()};node.addEventListener('pointermove',move);node.addEventListener('pointerup',up);node.addEventListener('pointercancel',up)});
...
function renderMusicMaxHome(){
  ...
  const liked=[...all].filter(x=>x.liked).sort((a,b)=>(b.score||0)-(a.score||0));
...
function renderPlayer(){
  ...
  const media=yt?`<div id="ytPlayerHost" class="yt-player yt-player-isolated-placeholder"></div>`:'<audio controls></audio>';
  ...
  r.querySelector('[data-ma=shuffle]').onclick=()=>{music.shuffle=!music.shuffle;music.shuffleRecent=[];music.shuffleBack=[];save('teslaWaze:musicShuffle:v1',music.shuffle);r.querySelector('[data-ma=shuffle]').classList.toggle('primary',music.shuffle)};
...
/* MUSIC_NAV_RESUME_WATCHDOG_REMOVED_V91 */
function syncMediaSession(){updateMiniSeek()}
function wireAudio(){if(!music.audio)return;music.audio.onloadedmetadata=()=>{const d=Number(music.audio?.duration||0);if((Number.isFinite(d)&&d>0&&d<MUSIC_MIN_SECONDS)||(!Number.isFinite(d)&&d!==0)){if(invalidateCurrentMedia('short-or-invalid-audio',d)){try{music.audio.pause()}catch{};setTimeout(()=>mnext('auto'),0);return}}if(music.current&&d>0){mt(music.current).durationSeconds=Math.round(d);save(LS.music,music.profile)}};music.audio.onended=()=>{if(Date.now()<(music.suppressEndedUntil||0))return;if(music.current)mev('complete',music.current);if(music.autoNext&&!music.userPaused){music.wantsPlayback=true;setMusicPlaying(true);mnext('auto')}else{music.wantsPlayback=false;setMusicPlaying(false)}};music.audio.onplay=()=>{music.userPaused=false;music.wantsPlayback=true;if(!music.playingSince)music.playingSince=Date.now();setMusicPlaying(true);if(music.manualNavPending){if(music.manualNavTimer)clearTimeout(music.manualNavTimer);music.manualNavTimer=setTimeout(()=>{music.manualNavTimer=null;drainManualMusicNav()},1500)}};music.audio.onpause=()=>{if(music.userPaused||!music.wantsPlayback)setMusicPlaying(false);else{setMusicPlaying(true);scheduleMusicResume()}};music.audio.ontimeupdate=()=>{updateMiniSeek()}}
function ensureMusicQueue(){
  const items=Object.values(music.profile.tracks).filter(isEligibleMusic).sort((x,y)=>(y.score+(isYoutubePreference(y)?3:0))-(x.score+(isYoutubePreference(x)?3:0)));
...
function refreshPersistentYoutubeUi(){
...
const TW_MUSIC_ENGINE_URL='https://raw.githack.com/lukaslejko-jpg/webstranka/tesla-waze-preview-v1/tesla-waze-preview/music-isolated-engine-v117.html?v=117';/* MUSIC_HARD_ISOLATION_V117 */
...
function mplay(t){
  ...
  const canReuseYoutube=!!(yt&&music.ytPlayer&&typeof music.ytPlayer.loadVideoById==='function');/* MUSIC_NEXT_REUSE_V99 */
  music.current=t;music.userPaused=false;music.wantsPlayback=true;music.playingSince=0;setMusicPlaying(true);syncMediaSession();music.started=Date.now();mev(replay?'replay':'play',t);expandRelatedQueue(t);/* MUSIC_ENDLESS_DISCOVERY_V100 */
  if(canReuseYoutube){
...
function mnext(reason='manual'){
  if(reason==='manual'&&!beginMusicNav())return false;
...
async function searchYoutube(q){const r=await fetch('https://dimvegkezslqjtsxdohp.supabase.co/functions/v1/twyoutubesearch?q='+encodeURIComponent(q),{cache:'no-store'});if(!r.ok)throw new Error('youtube search '+r.status);const d=await r.json();return Array.isArray(d.items)?d.items:[]}/* MUSIC_PUBLIC_YOUTUBE_SEARCH_V84 */
...
function setMusicSearchMode(active){document.querySelector('.music-tabs')?.classList.toggle('hidden',active);$('musicList')?.classList.toggle('hidden',active);const shell=document.querySelector('.music-shell'),results=$('musicSearchResults'),card=results?.closest('.music-card');if(card&&!card.id)card.id='musicSearchCard';shell?.classList.toggle('music-searching',!!active);if(!active&&results)results.innerHTML='';if(shell?.classList.contains('music-maximized')){const home=$('musicMaxHome');if(home)home.classList.toggle('hidden',!!active);ensureMaxMusicHeaderSearch();const back=$('musicHeaderBackBtn');if(back)back.classList.toggle('hidden',!active)}}
async function musicSearch(){const q=$('musicSearch').value.trim(),o=$('musicSearchResults');if($('musicHeaderSearchInput')&&$('musicHeaderSearchInput').value!==q)$('musicHeaderSearchInput').value=q;if(!q){setMusicSearchMode(false);return}rememberMusicSearch(q);setMusicSearchMode(true);o.innerHTML='<div class="music-empty">Hľadám na YouTube a v ďalších zdrojoch…</div>';const nq=norm(q),profile=Object.values(music.profile.tracks).filter(t=>isYoutubePreference(t)&&!t.disliked),matching=profile.filter(t=>norm(`${t.artist} ${t.title}`).includes(nq)).sort((a,b)=>b.score-a.score),[ytResult,freeResult]=await Promise.allSettled([searchYoutube(q),resolveMusic(q)]),found=ytResult.status==='fulfilled'?ytResult.value:[],resolved=freeResult.status==='fulfilled'?freeResult.value.filter(t=>t.streamUrl&&isEligibleMusic(t)):[],seen=new Set(),take=items=>items.filter(t=>{const k=t.youtubeId?`youtube:${t.youtubeId}`:`${norm(t.artist)}::${norm(t.title)}`;if(seen.has(k))return false;seen.add(k);return true}),youtube=take([...found,...matching]),seedArtists=new Set(youtube.slice(0,4).map(t=>norm(t.artist)).filter(Boolean)),recommended=youtube.length?take(profile.filter(t=>!norm(`${t.artist} ${t.title}`).includes(nq)&&seedArtists.has(norm(t.artist))).sort((a,b)=>(b.score+(b.liked?4:0))-(a.score+(a.liked?4:0))).slice(0,8)):[],other=take(resolved),a=[...youtube,...recommended,...other];if(ytResult.status==='rejected')console.warn('YouTube vyhľadávanie zlyhalo:',ytResult.reason?.message||ytResult.reason);if(freeResult.status==='rejected')console.warn('Vyhľadávanie ďalších zdrojov zlyhalo:',freeResult.reason?.message||freeResult.reason);music.queue=a;save(LS.queue,a);renderMusicGroups([{title:'YouTube',items:youtube},{title:'Podobné a odporúčané',items:recommended},{title:'Ďalšie prehrateľné zdroje',items:other}],o);if(ytResult.status==='rejected')o.insertAdjacentHTML('afterbegin','<div class="music-search-warning">YouTube vyhľadávanie sa teraz nepodarilo načítať. Skúste to znova o chvíľu.</div>')}
async function musicSources(t){const o=$('musicSources'),q=`${t.artist||''} ${t.title||''}`.trim();o.innerHTML='<div class="music-card">Hľadám prehrateľný zdroj v aplikácii…</div>';let free=[];try{free=(await resolveMusic(q)).filter(x=>x.streamUrl&&isEligibleMusic(x))}catch{}o.innerHTML=`<div class="music-card"><b>${esc(t.artist||'')} — ${esc(t.title||'')}</b>${free.length?free.map(musicCard).join(''):'<div class="music-empty">Prehrateľná verzia sa v dostupných zdrojoch nenašla.</div>'}</div>`;o.querySelectorAll('[data-mplay]').forEach(b=>b.onclick=()=>{const x=free.find(y=>mt(y).id===b.dataset.mplay);if(x){music.queue=free;mplay(x)}})}
...
/* MUSIC_GAPLESS_HANDOFF_V16 */
/* MUSIC_PREGAPLESS_CORE_V42 */
