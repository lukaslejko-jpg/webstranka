...
await fs.writeFile('public/sw.js',`/* Tesla Music V41: online only; member/admin layer is network-authenticated. */\nself.addEventListener('install',()=>self.skipWaiting());\nself.addEventListener('activate',e=>e.waitUntil((async()=>{for(const k of await caches.keys()){if(k.startsWith('tesla-music-pwa-'))await caches.delete(k);}await self.clients.claim();})()));\nself.addEventListener('fetch',e=>{if(new URL(e.request.url).origin===self.location.origin)e.respondWith(fetch(e.request));});\n`);
await fs.writeFile('public/version.json',JSON.stringify({release:41,source:SOURCE,production:'https://tesla-waze-piped.vercel.app',search:'/api/youtube-search',searchRelease:40,offline:false,airplay:false,icon:'/icons/music-v39-180.png',controls:'mobile-controls-v40.js',members:'member-v41.js',profileSync:'synology',admin:true,nativeQueue:'ios-standalone-auto',oneTapPlay:true}));
console.log('Tesla Music V41: V40 playback/search preserved; Synology member/admin profile layer added.');
