...
await fs.writeFile('public/sw.js',`/* Tesla Music V40: online only; no offline module. */\nself.addEventListener('install',()=>self.skipWaiting());\nself.addEventListener('activate',e=>e.waitUntil((async()=>{for(const k of await caches.keys()){if(k.startsWith('tesla-music-pwa-'))await caches.delete(k);}await self.clients.claim();})()));\nself.addEventListener('fetch',e=>{if(new URL(e.request.url).origin===self.location.origin)e.respondWith(fetch(e.request));});\n`);
await fs.writeFile('public/version.json',JSON.stringify({release:40,source:SOURCE,production:'https://tesla-waze-piped.vercel.app',search:'/api/youtube-search',searchRelease:40,offline:false,airplay:false,icon:'/icons/music-v39-180.png',controls:'mobile-controls-v40.js',nativeQueue:'ios-standalone-auto',oneTapPlay:true}));
console.log('Tesla Music V40: mobile controls, resilient anonymous search, checksum-verified selected PNG icons and online-only worker built.');
