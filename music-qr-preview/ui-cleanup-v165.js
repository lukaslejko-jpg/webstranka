...
document.addEventListener('click',function(e){if(!isVideoButton(e.target))return;var p=window.player;if(!p)return;var id='',time=0,state=-1;try{var d=p.getVideoData?p.getVideoData():null;id=d&&d.video_id?d.video_id:'';time=p.getCurrentTime?p.getCurrentTime():0;state=p.getPlayerState?p.getPlayerState():-1}catch(x){}
 setTimeout(function(){try{var q=window.player;if(!q||!id)return;var d2=q.getVideoData?q.getVideoData():null,now=d2&&d2.video_id?d2.video_id:'';if(now!==id&&q.loadVideoById){q.loadVideoById({videoId:id,startSeconds:Math.max(0,time)});setTimeout(function(){try{if(state===1&&q.playVideo)q.playVideo();else if(q.pauseVideo)q.pauseVideo()}catch(x){}},350)}else{var current=q.getCurrentTime?q.getCurrentTime():time;if(Math.abs(current-time)>1.5&&q.seekTo)q.seekTo(Math.max(0,time),true);if(state===1&&q.playVideo)q.playVideo();else if((state===2||state===5)&&q.pauseVideo)q.pauseVideo()}}catch(x){}},120);
},true);
