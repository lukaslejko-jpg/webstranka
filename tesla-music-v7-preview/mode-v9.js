(()=>{
  const qs=new URLSearchParams(location.search),EMBED=qs.get('embed')==='1',MAP=qs.get('map')==='1'||EMBED;
  ...
  function requestMiniBottomRight(){if(!EMBED||!isMini()||HOST===window)return;setTimeout(()=>{try{HOST.postMessage({type:'tesla-music-move',dx:999999,dy:999999},ORIGIN)}catch{}},0)}
  ...
  function notify(force=false){if(!EMBED||HOST===window)return;const payload={type:'tesla-music-ui-state',mode:isMini()?'mini':'full',video:videoOn},sig=payload.mode+'|'+payload.video;if(!force&&sig===lastSent)return;lastSent=sig;try{HOST.postMessage(payload,ORIGIN)}catch{}}
  ...
  function flushMiniResize(){rafMove=0;const dx=pendingMiniDx,dy=pendingMiniDy;pendingMiniDx=0;pendingMiniDy=0;if(!dx&&!dy)return;try{HOST.postMessage({type:'tesla-music-resize',anchor:'tl',dx,dy},ORIGIN)}catch{}}
  if(EMBED){
    ...
    head.addEventListener('pointermove',e=>{if(!drag||drag.id!==e.pointerId)return;e.preventDefault();e.stopImmediatePropagation();const dx=e.clientX-drag.x,dy=e.clientY-drag.y;drag.x=e.clientX;drag.y=e.clientY;try{HOST.postMessage({type:'tesla-music-move',dx,dy},ORIGIN)}catch{}},true);
