(()=>{
  ...
  function normalizeEmbed(){if(!EMBED)return;if(pw.classList.contains('max'))pw.classList.remove('max');const s=pw.style;if(s.getPropertyValue('left')!=='0px')s.setProperty('left','0px','important');if(s.getPropertyValue('top')!=='0px')s.setProperty('top','0px','important');if(s.getPropertyValue('width')!=='100%')s.setProperty('width','100%','important');if(s.getPropertyValue('height')!=='100%')s.setProperty('height','100%','important')}
  function notify(){if(!EMBED||window.parent===window)return;const payload={type:'tesla-music-ui-state',mode:isMini()?'mini':'full',video:videoOn},sig=payload.mode+'|'+payload.video;if(sig===lastSent)return;lastSent=sig;try{parent.postMessage(payload,ORIGIN)}catch{}}
  function syncUi(){normalizeEmbed();applyVideo();notify()}
  function startup(){if(MAP&&typeof window.teslaMusicSetMini==='function')window.teslaMusicSetMini();if(MAP)videoOn=false;ensureCloseButton();ensureVideoButton();syncUi()}
  setTimeout(startup,0);setTimeout(startup,350);
  window.teslaMusicSetVideo=v=>{videoOn=!!v;if(isMini())videoOn=false;sessionStorage.setItem('teslaMusic:videoVisible:v1',videoOn?'1':'0');syncUi()};
  window.teslaMusicToggleVideo=()=>{if(!isMini())window.teslaMusicSetVideo(!videoOn)};
  window.addEventListener('message',e=>{if(e.origin!==ORIGIN||!e.data||e.data.type!=='tesla-music-command')return;const a=e.data.action;if(a==='mini')window.teslaMusicSetMini?.();else if(a==='full')window.teslaMusicSetFull?.();else if(a==='video-toggle')window.teslaMusicToggleVideo?.();else if(a==='video-off')videoOn=false;else if(a==='video-on'&&!isMini())videoOn=true;setTimeout(syncUi,0)});
  [pmin,pmax].filter(Boolean).forEach(b=>b.addEventListener('click',()=>setTimeout(syncUi,0),false));
  if(EMBED){
  ...
  ensureCloseButton();ensureVideoButton();syncUi();
})();