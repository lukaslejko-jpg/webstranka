(()=>{
  ...
  function applyMode(){const mini=isMini();document.body.classList.toggle('music-full',EMBED&&!mini);if(EMBED&&mini){const s=pw.style;pw.classList.remove('max');if(s.getPropertyValue('left')!=='0px')s.setProperty('left','0px','important');if(s.getPropertyValue('top')!=='0px')s.setProperty('top','0px','important');if(s.getPropertyValue('width')!=='100%')s.setProperty('width','100%','important');if(s.getPropertyValue('height')!=='100%')s.setProperty('height','100%','important')}else if(EMBED&&!mini){pw.classList.remove('max');pw.style.removeProperty('left');pw.style.removeProperty('top');pw.style.removeProperty('width');pw.style.removeProperty('height')}}
  ...
  if(EMBED&&pmax){pmax.onclick=e=>{e.preventDefault();e.stopPropagation();if(isMini())window.teslaMusicSetFull?.();else window.teslaMusicSetMini?.();setTimeout(()=>syncUi(true),0)}}
  if(pmin)pmin.addEventListener('click',()=>setTimeout(()=>syncUi(true),0),false);
  if(EMBED){
